	...
	for i := 0; i < len(data); i++ {
		...
		for i := 0; i < len(newbills); i++ {
			if newbills[i].Paid == false {
				message := "Your%20Bill%20Amount%20for%20RR%20Number%20" + newbills[i].RRNum + "%20is%20Rupees%20" + fmt.Sprint(newbills[i].NetAmtDue) + ".%20Please%20pay%20before%20" + newbills[i].DueDate + "%20to%20avoid%20panalty.%20%20Thank%20You"
