...
func EmailService(w http.ResponseWriter, r *http.Request) {
	...
	//db.Where(&Bill{RRNum:data.RRNum, Paid:false}).First(&newbill)
	// fmt.Println(newdata)
...
func MailtoAll(w http.ResponseWriter, r *http.Request) {
	...
	for i:=0;i<len(data);i++  {
		...
		for i:=0;i<len(newbills);i++ {
			if newbills[i].Paid == false {
				// fmt.Println(newbills[i].Email, newbills[i].BillNum, newbills[i].Paid)
				// from here
				emailAuth := smtp.PlainAuth("", emailSender, passWord, hostUrl)

				// mydata := newbills[i].Email
				//mydata := strconv.FormatFloat(newbills[i].NetAmtDue, 'f', 10, 64)

				//billnum := strconv.FormatUint(uint64(newbills[i].BillNum),64)
				//fmt.Println(billnum)
				//phone := strconv.FormatUint(uint64(newbills[i].Phone),64)
				//fmt.Println(phone)
				//amt:= strconv.FormatFloat(newbills[i].NetAmtDue,'f',2,64)
				//fmt.Println(amt)
				// phonedata := strconv.FormatUint(data.Phone, 64)
				...
				//msg := []byte("To:" + newbills[i].Email + "\r\n" +
				//	"Subject: BESCOM Electricity Bill \r\n" +
				//	"\r\n" +
				//	"RR Num: "+newbills[i].RRNum + "\r\n" + "Name: "+newbills[i].Name+ "\r\n"+ "Address: "+newbills[i].Address + "\r\n" +"Email: "+newbills[i].Email + "\r\n"+ "\r\n"+"Bill Amount: "+mydata + "\r\n")
	...
	//db.Where(&Users{RRNum:}).Find(&newbills)
	 //fmt.Println(len(newbills))
	/*
